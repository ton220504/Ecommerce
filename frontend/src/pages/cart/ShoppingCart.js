...
class ShoppingCart extends Component {
  ...
  getAuth(token) {
    axios
      ...
      .then((result) => {
        ...
        else this.getShoppingCartList(result.data.user.id);
      })
      .catch((error) => {
        console.log(error);
  ...
  handleChange(e) {
    if (e.target.type === "checkbox") {
      ...
      // axios
      //   .put(`${ip}/product/cart-list/${e.target.id}`, {
      //     quantity: quantity,
      //   })
      //   .then((response) => {
      ...
      //     }
      //   });
      axios.put(`${ip}/product/cart-list/${e.target.id}`, {
        quantity: quantity,
      })
      .then((response) => {
        if (response.status === 200) {
  ...
  //     axios
  //       .delete(`${ip}/product/cart-list/${id}`, {
  //         headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
  //       })
  //       .then((response) => {
  //         if (response.status === 200) {
  ...
  handleDelete(e) {
    ...
    if (this.state.userId) {
      axios
        ...
        .then((response) => {
          if (response.status === 200) {
            ...
            this.props.updateCartCount(list.length);
          }
        });
    } else {
      let items = JSON.parse(localStorage.getItem("cartList"));
      ...
      // Cập nhật số lượng giỏ hàng ngay lập tức
      this.props.updateCartCount(items.length);
    }
  }
  ...
  handleCheckout(e) {
    ...
    if (id !== 0) selectCheckout = [id];
    else selectCheckout = this.state.selectedList;
                    ...
                    <div key={item.id} className="cart-item">
                      <div className="media cart-item-box">
                        ...
                        <img
                          ...
                          src={`./img/${JSON.parse(item.stock.product.photo)[0]
                            }`}
                          alt={JSON.parse(item.stock.product.photo)[0]}
                        />
                        <div className="media-body cart-item-body">
                          <h5 className="mt-0 product-name">
                            {item.stock && item.stock.product ? (
                        ...
                        <Link onClick={this.handleCheckout} to={"/checkout"}>
                          <button id={item.id} className="item-checkout-btn">
                            checkout
                          </button>
                        </Link>
                      </div>
